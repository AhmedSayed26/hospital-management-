...
export default function EditDoctor({ doctor, open, onOpenChange, onSaved }) {
  const [showPassword, setShowPassword] = useState(false);
  const initialValues = useMemo(() => doctorToFormValues(doctor), [doctor]);
  const fieldIdPrefix = `edit-doctor-${doctor?.id ?? "new"}`;

  const formik = useFormik({
    enableReinitialize: true,
    initialValues,
    validationSchema: getDoctorValidationSchema({ passwordRequired: false }),
    onSubmit: async (values, { setSubmitting, resetForm }) => {
      if (!doctor?.id) return;

      try {
        await DoctorService.UpdateDoctor(doctor.id, toDoctorPayload(values));

        toast.add({
          type: "success",
          title: "Doctor updated",
          description: "The doctor was saved successfully",
        });
        ...
        toast.add({
          type: "error",
          title: "Error",
          description: getErrorMessage(error, "Failed to update doctor"),
        });
      } finally {
        setSubmitting(false);
      }
      ...
      <DialogContent className="sm:max-w-lg">
        <form onSubmit={formik.handleSubmit} className="grid gap-4">
          ...
          <div className="grid max-h-[60vh] gap-3 overflow-y-auto px-1 py-1">
            ...
            <div className="space-y-1.5">
              <label htmlFor={`${fieldIdPrefix}-experience`} className="text-sm font-medium">
                Years of experience
              </label>
              <Input
          ...
          <DialogFooter>
            <Button type="submit" disabled={formik.isSubmitting || !doctor?.id}>
