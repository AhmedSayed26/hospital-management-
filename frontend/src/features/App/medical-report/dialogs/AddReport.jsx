...
export default function AddReport({ onCreated }) {
  ...
  const formik = useFormik({
    ...
    initialValues: {
      ...emptyMedicalReportValues,
      doctorId: user?.id != null ? String(user.id) : "",
    },
    validationSchema: getMedicalReportValidationSchema(),
    onSubmit: async (values, { setSubmitting, resetForm }) => {
      try {
        await MedicalReportService.CreateMedicalReport(toMedicalReportPayload(values));

        toast.add({
          type: "success",
          title: "Medical report created",
          description: "The medical report was added successfully",
        });
        ...
        toast.add({
          type: "error",
          title: "Error",
          description: getErrorMessage(error, "Failed to create medical report"),
        });
      } finally {
        setSubmitting(false);
      }
        ...
        <form onSubmit={formik.handleSubmit} className="grid gap-4">
          ...
          <div className="grid max-h-[60vh] gap-3 overflow-y-auto px-1 py-1">
            <div className="space-y-1.5">
              <label htmlFor="add-report-title" className="text-sm font-medium">
                Report title
              </label>
              <Input
                id="add-report-title"
            ...
            <div className="space-y-1.5">
              <label htmlFor="add-report-date" className="text-sm font-medium">
                Report date
              </label>
              <DataPicker
                id="add-report-date"
            ...
            <div className="space-y-1.5">
              <label htmlFor="add-report-content" className="text-sm font-medium">
                Content
              </label>
              <textarea
                id="add-report-content"
                name="content"
                rows={4}
            ...
            <div className="space-y-1.5">
              ...
              <Select
                ...
                onValueChange={(value) => formik.setFieldValue("medicalRecordId", value ?? "")}
          ...
          <DialogFooter>
            <Button type="submit" disabled={formik.isSubmitting || loading}>
              {formik.isSubmitting ? "Saving..." : "Add Medical Report"}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
